...
- **What**: Each Cascade conversation can run in its own git worktree — a full copy of the repo on a separate branch, isolated from the main workspace.
...
- Scoped per directory — Cascade discovers and follows them automatically.
...
┌─────────────────────────────────────────────────┐
│                  SPEC PHASE                      │
│  spec.md → tasks.md → architecture decisions     │
└──────────────────────┬──────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────┐
│              MERGE & REVIEW                      │
│  Merge worktree → PR → human review             │
└─────────────────────────────────────────────────┘
```
...
.windsurf/
├── workflows/
│   ├── spec-loop.md          # Main orchestration workflow
│   ├── implement-task.md     # Single task implementation
│   ├── verify-task.md        # Test/lint/build verification
│   └── merge-and-review.md   # Final merge workflow
├── skills/
│   └── spec-driven-dev/
│       ├── SKILL.md           # Auto-invoked skill for SDD
│       ├── spec-template.md   # Spec template
│       └── task-template.md   # Task breakdown template
├── hooks.json                 # Lifecycle hooks
└── rules/
    └── spec-loop-rules.md     # Global rules for loop behavior
```
...
1. **Cascade 1** (Worktree): "Implement Task 3 - User authentication" → `/implement-task`
2. **Cascade 2** (Worktree): "Implement Task 4 - Dashboard UI" → `/implement-task`
3. **Cascade 3** (Worktree): "Implement Task 5 - API endpoints" → `/implement-task`
...
- Context window fills up over long sessions — Cascade may lose track of earlier context
...
### Strategy 4: Plan Mode → Implement Pipeline
...
3. Click "Implement" → Cascade switches to Code mode and executes
...
|-----------|----------|-------------------|--------|-------|
| Parallel agents | ✅ Worktrees + Simultaneous Cascades | ✅ Multiple terminals | ❌ Single agent | ✅ Worktrees |
| Spec-driven loop | 🟡 Via workflows (manual setup) | ✅ Ralph automates | ❌ Manual | ✅ Native |
| Auto-execution | ✅ Turbo mode | ✅ `--dangerously-skip-permissions` | 🟡 Limited | ✅ Sandboxed |
| Lifecycle hooks | ✅ Rich hook system | ❌ None | ❌ None | ❌ None |
| Programmatic API | ❌ No | ✅ CLI-driven | ❌ No | ✅ CLI + API |
| Auto-continue | ✅ Built-in | ✅ Inherent (CLI) | 🟡 Manual | ✅ Inherent |
| Inter-agent comms | ❌ No | 🟡 Via filesystem | ❌ No | 🟡 Via filesystem |
| Plan → Execute | ✅ Plan Mode | 🟡 Manual | 🟡 Manual | ❌ No |
...
│   ├── plan-spec.md     # Generate spec from requirements
│   └── review-merge.md  # Review and merge workflow
├── skills/
│   └── spec-dev/
│       └── SKILL.md     # SDD skill with templates
├── hooks.json           # Worktree setup + progress logging
└── rules/
    └── autonomy.md      # Rules for autonomous behavior
```
